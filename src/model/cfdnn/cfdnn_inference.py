...
from model.cfdnn.cfdnn import CFDNN as Net
...
class Inference:
    ...
    def rank(self, query, docs:dict):
        ...
        for i in doc_embeds:
            print(i[:10])
        with torch.no_grad():
            result = self.model(torch.Tensor(np.array([query_embed]*len(doc_embeds))), torch.Tensor(np.array(doc_embeds)))
